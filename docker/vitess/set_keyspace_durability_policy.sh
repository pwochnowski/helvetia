...
for keyspace in $keyspaces; do
  echo "Setting durability policy for $keyspace..."
  max_attempts=30
  attempt=0
  while [ $attempt -lt $max_attempts ]; do
    if vtctldclient --server vtctld:$grpc_port SetKeyspaceDurabilityPolicy --durability-policy=semi_sync $keyspace 2>/dev/null; then
