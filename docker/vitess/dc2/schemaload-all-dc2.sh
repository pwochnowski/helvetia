...
# Schema loading script for all Helvetia keyspaces - DC2 version
...
# Define keyspaces and their configurations for DC2
# Format: keyspace:target_tablet:schema_files:vschema_file
# DC2 tablets use UIDs starting with 2xxx
KEYSPACE_CONFIGS=(
  "user_keyspace:cell1-0000002101:create_user.sql:vschema/user_vschema.json"
  "article_keyspace:cell1-0000002201:create_article.sql:vschema/article_vschema.json"
  "read_keyspace:cell1-0000002301:create_read.sql:vschema/read_vschema.json"
  "beread_keyspace:cell1-0000002401:create_beread.sql:vschema/beread_vschema.json"
  "popularrank_keyspace:cell1-0000002501:create_popularrank.sql:vschema/popularrank_vschema.json"
)
...
load_schema_for_keyspace() {
  ...
  echo ""
  echo "=========================================="
  echo "Loading schema for ${keyspace} (DC2)"
  echo "=========================================="
  ...
  while [ $attempt -lt $max_attempts ]; do
    if vtctldclient --server vtctld_dc2:${GRPC_PORT} GetTablet $targettab 2>/dev/null; then
  ...
  attempt=0
  while [ $attempt -lt $max_attempts ]; do
    primary_count=$(vtctldclient --server vtctld_dc2:${GRPC_PORT} GetTablets --keyspace=$keyspace 2>/dev/null | grep -c "primary" || echo "0")
  ...
  while [ $vschema_attempt -lt $vschema_max ]; do
    if vtctldclient --server vtctld_dc2:${GRPC_PORT} ApplyVSchema --vschema-file /script/${vschema_file} $keyspace 2>/dev/null; then
...
if [ -f /tmp/schemaload_all_dc2_done ]; then
...
# Mark as complete
touch /tmp/schemaload_all_dc2_done

echo ""
echo "=========================================="
echo "✓ All schemas loaded successfully (DC2)!"
echo "=========================================="
