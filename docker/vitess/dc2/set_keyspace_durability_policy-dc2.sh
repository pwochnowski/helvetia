...
# Set keyspace durability policy for Helvetia DC2
...
for keyspace in $keyspaces; do
  ...
  while [ $attempt -lt $max_attempts ]; do
    if vtctldclient --server vtctld_dc2:$grpc_port SetKeyspaceDurabilityPolicy --durability-policy=$durability_policy $keyspace 2>/dev/null; then
